...
export default function VehiculeDetail() {
  ...
  if (dateDebut && dateFin && vehicule) {
    ...
    if (end < start) {
      ...
      prixTotal = diffDays * (Number(vehicule.prix_jour) + optionsPrixJournalier);
    }
  }
    ...
    return (
      <main className="max-w-7xl mx-auto px-4 py-24 text-center">
        ...
        <Link to="/" className="text-blue-600 hover:text-blue-800 font-medium flex items-center justify-center">
          <ArrowLeft className="h-5 w-5 mr-2" aria-hidden="true" /> Retour au catalogue
        </Link>
      </main>
    );
  }
    ...
    <main className="bg-[#F9F9F5] min-h-screen pb-32">
      <header className="bg-white border-b border-gray-200 pt-6 pb-6 px-4 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link to="/" className="inline-flex items-center text-sm font-bold uppercase tracking-widest text-gray-400 hover:text-blue-500 transition-colors" aria-label="Retourner au catalogue">
            <ArrowLeft className="h-4 w-4 mr-2" aria-hidden="true" /> Retour
          </Link>
          <span className="text-xs uppercase tracking-widest font-bold text-blue-500" aria-hidden="true">
            Configuration
          </span>
        </div>
      </header>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <section className="lg:w-3/5" aria-labelledby="vehicule-title">
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
              ...
              <div className="p-8">
                <h1 id="vehicule-title" className="text-4xl font-serif font-extrabold text-gray-900 leading-tight mb-2">
                  {vehicule.marque} <span className="font-light">{vehicule.modele}</span>
                </h1>
                <p className="text-gray-500 flex items-center mb-8">
                  Plaque : <span className="font-mono bg-gray-100 px-2 py-1 rounded ml-2">{vehicule.immatriculation}</span>
                </p>
